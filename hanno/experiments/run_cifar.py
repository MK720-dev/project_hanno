"""
First runnable CIFAR-10 experiment for Hanno / HannoNet1.

Goal
----
This script keeps the same Hanno structure used in the MNIST phase but
replaces the optimizee with a small CNN trained on CIFAR-10.

The first proof-of-concept CIFAR-10 setup is intentionally modest:
- small CNN family
- Adam backbone
- moderate horizon
...
From the project root, run:
    ...
    python -m hanno.experiments.run_cifar
...
from hanno.tasks.cifar10 import Cifar10Task, Cifar10TaskConfig


@dataclass(frozen=True)
class ResourceSchedule:
    """
    Lightweight training-budget preset for CIFAR-10 transfer experiments.
    """
...
@dataclass(frozen=True)
class CifarTransferExperiment:
    """
    One CIFAR-10 experiment bundle containing a training family, a held-out
    family, and the episode-budget settings used for the run.
    """
...
DEFAULT_RESOURCE_SCHEDULES: tuple[ResourceSchedule, ...] = (
    ResourceSchedule(name="short_low", horizon=60, num_episodes=24),
    ResourceSchedule(name="medium_mid", horizon=90, num_episodes=36),
    ResourceSchedule(name="long_high", horizon=120, num_episodes=54),
)


DEFAULT_EXPERIMENTS: tuple[CifarTransferExperiment, ...] = (
    CifarTransferExperiment(
        name="cnn_transfer_baseline",
        train_variants=("small", "wide", "deep"),
        eval_variants=("narrow", "bottleneck", "very_wide"),
        resource_schedule=DEFAULT_RESOURCE_SCHEDULES[1],
    ),
    CifarTransferExperiment(
        name="cnn_transfer_long_horizon",
        train_variants=("small", "wide", "deep"),
        eval_variants=("narrow", "bottleneck", "very_wide"),
...
def run_episode(
    ...
    """
    Run one full CIFAR-10 rollout under the current controller.
    """
    ...
    metric_class = None
    try:
        model = env.task_state.parameters
...
def build_env(device: str, model_variant: str, horizon: int) -> TrainingEnv:
    """
    Assemble one CIFAR-10 environment instance for the requested architecture
    and resource setting.
    """

    task = Cifar10Task(
        config=Cifar10TaskConfig(
            horizon=horizon,
            batch_size=64,
            train=True,
            data_root="./data",
...
def train_transfer_family(
    ...
    device: str,
    experiment: CifarTransferExperiment,
    base_seed: int,
) -> None:
    """
    Train one controller across a rotating family of CIFAR-10 CNN
    architectures.
    """
...
def evaluate_transfer_family(
    ...
    """
    Evaluate the trained controller on a held-out CIFAR-10 CNN family without
    updating the policy.
    """
...
def main() -> None:
    """
    Assemble and run the first HannoNet1 CIFAR-10 transfer experiments.
    """
    ...
    for experiment_idx, experiment in enumerate(DEFAULT_EXPERIMENTS, start=1):
        ...
        train_transfer_family(
            ...
            base_seed=12000 + (experiment_idx * 1000),
        )
        evaluate_transfer_family(
            ...
            base_seed=18000 + (experiment_idx * 1000),
        )
